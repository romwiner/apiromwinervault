...
// ============================================
// 🔓 DESBLOQUEADOR MÁGICO DEL LOGIN
// ============================================

document.addEventListener('DOMContentLoaded', function() {
  setTimeout(function() {
    
    // 1. FORZAR que el botón azul funcione
    const btnShowLogin = document.getElementById('btnShowLogin');
    if (btnShowLogin) {
      btnShowLogin.style.pointerEvents = 'auto';
      btnShowLogin.style.cursor = 'pointer';
      btnShowLogin.style.zIndex = '999999';
      btnShowLogin.style.position = 'relative';
      btnShowLogin.disabled = false;
      
      // Asegurar que tenga el evento click
      btnShowLogin.onclick = function(e) {
        ...
        if (loginContainer) {
          ...
          loginContainer.style.zIndex = '999999';
          loginContainer.style.position = 'relative';
          loginContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
        
        // Auto-rellenar usuario
        const identidadGuardada = localStorage.getItem('arv_identity');
        if (identidadGuardada) {
          const loginUsername = document.getElementById('loginUsername');
          if (loginUsername) {
            loginUsername.value = identidadGuardada;
          }
        }
        
        // DESBLOQUEAR los inputs del login
        setTimeout(function() {
          const inputsLogin = document.querySelectorAll('#loginContainer input');
      ...
      console.log('✅ Botón azul configurado');
    }
    
    // 2. DESBLOQUEAR todo el formulario de login
    const loginContainer = document.getElementById('loginContainer');
    if (loginContainer) {
      loginContainer.querySelectorAll('*').forEach(function(el) {
        el.style.pointerEvents = 'auto';
        el.style.zIndex = '999999';
      });
    }
    
    // 3. ELIMINAR cualquier overlay bloqueante
    document.querySelectorAll('.overlay, .modal-backdrop, .blocker, [style*="pointer-events: none"]').forEach(function(el) {
      if (!el.closest('#loginContainer') && !el.closest('#digitalIdentityPanel')) {
        el.style.pointerEvents = 'none';
        el.style.zIndex = '1';
      }
    });
    
    console.log('✅ Sistema de login completamente desbloqueado');
